            ...
            """\
{
    ...
    %(to_name)s = CALL_METHOD_WITH_ARGS%(arg_size)d(%(called_name)s, %(called_attribute_name)s, call_args);
}
"""
            % {
...
def _getInstanceCallCodeFromTuple(
    ...
    emit(
        """\
%(to_name)s = CALL_METHOD_WITH_ARGS%(arg_size)d(%(called_name)s, %(called_attribute_name)s, &PyTuple_GET_ITEM(%(arg_tuple)s, 0));
"""
        % {
            "to_name": to_name,
            "arg_size": arg_size,
